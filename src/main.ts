...
export const setAppTitleFromLocation = (location: string, template: string) => {
  ...
  const { oidcUser } = useOidcUser();
  const { name, axa_uid_racf } = (oidcUser as any) || {};
console.log(oidcUser,"ppppppppppppppppppp")
{
    "sub": "B609EN",
    "name": "MILADI Zied",
    "axa_type": "2",
...
gma demande cest gerer les droite si on trouve ADMIN  on lui donne accer a longlet admin si ilya pas  il ne laffiche pas et il peux pas acceder 
