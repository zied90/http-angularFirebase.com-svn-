...
import React, { ChangeEvent, ForwardedRef, useEffect, useMemo, useState } from "react";
...
import { getTemplates, getTemplateById } from "@/api/apifunctions/logs/logs";
...
>(({ initialData, onData, onSubmit }, ref: ForwardedRef<HTMLFormElement>) => {
 ...
 const [initialTemplate, setInitialTemplate] = useState<TemplateOption | null>(null);
 // Fetch actions
 ...
 // Fetch the template by ID on mount if needed
 useEffect(() => {
   const fetchInitialTemplate = async () => {
     if (initialData?.templateId && !initialTemplate) {
       try {
         const { successData } = await getTemplateById(initialData.templateId);
         setInitialTemplate(successData);
       } catch (e) {
         console.error("Erreur lors du chargement du template initial :", e);
       }
     }
   };
   fetchInitialTemplate();
 }, [initialData?.templateId, initialTemplate]);
 const templateOptions: TemplateOption[] = templatesData?.successData || [];
 const allOptions = useMemo(() => {
   const set = new Map();
   (initialTemplate ? [initialTemplate, ...templateOptions] : templateOptions).forEach((tpl) =>
     set.set(tpl.id, tpl)
   );
 ...
 const handleSelectChange = (e: ChangeEvent<HTMLSelectElement>) => {
   const selectedApp = applications.find((app: App) => app.id === parseInt(e.target.value));
   if (selectedApp) {
     setFormData({
       ...formData,
...
<label htmlFor="typeahead-template" className="col-md-3 col-form-label p-2">
           Template
</label>
...
<CheckboxInput
       ...
       onChange={(e) => setFormData({ ...formData, isError: e.target.checked })}
       name="isError"
       id="errorCheckbox"
     />
</form>
...
interface Store {
  actions: any; // TODO
  setActions: (actions: IActionResponse) => void;
}

const actionStore = create<Store>((set) => ({
  actions: [],
  setActions: (actionsResponse) => {
    set({ actions: actionsResponse });
  },
}));
